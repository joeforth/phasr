...
from ultralytics.nn.modules.conv import (FeedbackConvLSTMBlock,TupleToTensor,SilenceChannel,Silence,SDM,ConcatSpe,FusionBlock)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c2 = args[1] - args[0]  # Output channels = c_end - c_start
        elif m is Silence:
            c2 = ch[f]  # This directly looks up the input channels and fixes the error
        elif m is ConcatSpe:
            c2 = ch[f[1]]
        elif m in (SDM, FusionBlock):
            # 1. Get c1 as a LIST of input channels (like Concat)
            c1 = [ch[x] for x in f]

            # 2. Calculate the SCALED output channels, c2 (like a base_module)
            c2 = make_divisible(args[0] * width, 8)

            # 3. Prepare the final arguments for your module's constructor
            args = [c1, c2, *args[1:]]
        else:
            c2 = ch[f]
